      ...
      <div :key="view" class="settings-view-wrapper">
        ...
        <div v-else-if="view === 'behavior'" class="settings-page">
          ...
          <div class="glass-card setting-group scrollable">
            ...
            <div class="setting-item">
              <div class="info">
                <h4>Фон экрана «Консоль»</h4>
                <p>Своя картинка под карточками на главном экране. {{ globalState.consoleBg ? 'Фон установлен.' : 'Не задан.' }}</p>
              </div>
              <div style="display: flex; gap: 8px;">
                <button class="action-btn" :disabled="bgBusy" @click="pickConsoleBg">{{ bgBusy ? '…' : 'Выбрать' }}</button>
                <button v-if="globalState.consoleBg" class="action-btn" :disabled="bgBusy" @click="clearConsoleBg">Убрать</button>
              </div>
            </div>
...
import { showAlert, showConfirm, globalState, setUiMode, setConsoleBg } from '../store';
...
const pickConsoleBg = async () => {
  bgBusy.value = true;
  try {
    const url = await (API as any).SetConsoleBg();
    if (url) setConsoleBg(url);
  } catch (e) {
...
const clearConsoleBg = async () => {
  try {
    await (API as any).ClearConsoleBg();
    setConsoleBg('');
  } catch (e) {
    await showAlert('Не удалось убрать фон: ' + e, 'Ошибка', true);
  }
};
