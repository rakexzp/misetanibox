        ...
        <div class="bg-editor" @click.stop>
          ...
          <div class="bg-editor-actions">
            <button class="action-btn accent-btn" @click="pickCardImage(editingCard)">
              {{ globalState.cardBgs[editingCard] ? 'Сменить картинку' : 'Выбрать картинку' }}
            </button>
            <button v-if="globalState.cardBgs[editingCard]" class="action-btn" @click="clearCard(editingCard)">Убрать</button>
          </div>
          ...
          <template v-if="globalState.cardBgs[editingCard]">
            ...
            <label class="bg-row"><span>Затемнение</span>
...
const cardStyle = (key: string) => {
  ...
  return {
    ...
    // 100 = cover (полностью покрывает карточку), выше = приближение
...
const pickCardImage = async (key: string) => {
  try {
    ...
    if (url) { setCardBg(key, url); optsFor(key); }
  } catch (e) {
