...
// singleInstanceLockFile держит flock на весь срок жизни процесса.
// Освобождается ОС автоматически при завершении процесса.
...
// acquireSingleInstanceLock пытается взять эксклюзивный flock на lock-файл.
// Возвращает false, если другой экземпляр уже держит лок.
func acquireSingleInstanceLock() bool {
	...
	if err != nil {
		...
		// Не блокируем запуск из-за проблем с lock-файлом
		return true
	}
...
func main() {
	// Восстановление после паники: последняя попытка снять системный прокси
	defer func() {
		if r := recover(); r != nil {
	...
	// Определяем dev-режим Wails (пропуск проверки единственного экземпляра)
	...
	// Стабилизация каталога данных / миграция — до загрузки чего-либо ещё
	...
	if hasFlag("--shutdown-existing") {
		// IPC-механизма завершения существующего экземпляра на Linux пока нет
		os.Exit(0)
	}
	...
	// Helper-служба (--install-helper / --uninstall-helper / --restart-helper) —
	// Windows-специфика, на Linux не поддерживается
	...
	// Единственный экземпляр через flock
	if !isDebugMode {
		...
		// Регистрация обработчиков сигналов ОС для аварийной очистки прокси
		installEmergencyProxyCleanup()
	}

	app := NewApp()

	// Дефолтный фоновый цвет — ночной (#111111), как в app.go
	...
	err := wails.Run(&options.App{
		...
		// На Linux трея пока нет: скрытое окно было бы не вернуть,
		// поэтому закрытие окна завершает приложение
		HideWindowOnClose: false,
		StartHidden:       true, // app.startup сам покажет окно, если запуск не «тихий»
...
// installEmergencyProxyCleanup регистрирует обработчики сигналов завершения
